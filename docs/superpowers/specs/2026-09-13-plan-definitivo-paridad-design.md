...
| `Max_Dist` del OSF es la distancia de la cumbre al píxel alertado **más lejano**, y `LAT/LON` es el píxel más caliente. **Medido 2026-09-14 con los TIF NRT** (`docs/audit_s139/DISTANCIA_MIROVA_VS_TIF.md`): la distancia que publica la web es `Max_Dist` (constante +0,113 km [0,072; 0,153], n = 171; pendiente con el VRP +0,211 km/década, ambas incompatibles con "más caliente"). Pero en la práctica las dos definiciones se separan 0,27 a 0,30 km en mediana y coinciden dentro de 1 km en el 87 % de las 52.364 filas chilenas del OSF; de las filas que el scraper esconde, sólo el 4 a 8 % tiene el píxel más caliente dentro del radio (Láscar hasta 20 %). | verificado (agente + re-medición propia sobre el OSF) | FALSO_POSITIVO sigue como **sin información** para el cráter (no es negativo). En el 92 a 96 % de esas filas el foco más caliente sí está fuera del radio: son un **control positivo fuerte** de nuestras detecciones `far` (métrica `far_ref`, plan F0 tarea 3.6). MIROVA mide desde un origen ~250 m al SO de la coordenada publicada (hallazgo lateral). Pregunta 11 a Coppola queda como confirmación, no como bloqueo. |
