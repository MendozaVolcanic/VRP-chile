...
Los `❌` salen del set-difference: `includeFarDistance` se lee en `buildAlertSummary:3053` y
`buildNRTTable:3297`, y ninguno está en el handler `:3505-3523`; `USE_F5_CORE` llega a la
barra vía `latestVRP→latestDetection:1409` y el handler `:3542` no la re-renderiza.

**Los `⊘` de la fila de sensores son la segunda familia**: no falta un `buildNRTTable()` en
el handler — es que **la tabla NRT global no lee `sensorVisible` en ninguna línea**, y el
**mapa de anomalías del detalle** tampoco (`hotspots` se arma en `index.html:2456` con
`.filter(r => (r.vrp_mw ?? r.vrp_mir_mw ?? 0) > 0)` y nada más). Apagar MODIS lo saca de las
tarjetas, la barra, los gráficos y la capa del mapa general, pero **el mapa del volcán que
estás mirando sigue dibujando sus puntos naranjas** y la tabla «Últimas detecciones NRT»
sigue listando filas `MODIS_*`. Como MODIS aporta ~450 puntos por volcán a 15-25 km sin
confirmación MIROVA (contexto S129), apagarlo es el gesto que un operador haría — y el panel
donde más importa es el que lo ignora. Verificado con `grep -n "isSensorVisible"`: 9
ocurrencias, ninguna entre las líneas 2456-2660 ni 3276-3378.

---

## 4. Persistencia — negativo limpio

No queda ningún desajuste de clave. Extraje todas las lecturas/escrituras de las cuatro
vistas y las pareé: `vrp_log_scale`, `vrp_include_far`, `vrp_footprint_markers`,
`vrp_f5_core`, `vrp_sensors_v3`, `diario_include_far`, `diario_f5_core`, `mosaico_f5_core`
— todas se escriben y se leen con el mismo string. El caso histórico (`vrp_sensors` escrito
vs `vrp_sensors_v3` leído) está corregido y documentado en `index.html:3019`.

Dos precisiones sobre el encargo: **es `sessionStorage`, no `localStorage`**
(`index.html:773-791`), o sea la config muere al cerrar la pestaña. Y las claves **no se
comparten entre vistas**: «🎯 Solo cráter» guarda en `vrp_include_far` en `index` y en
`diario_include_far` en `diario`, así que el mismo control con la misma etiqueta puede estar
en estados opuestos en dos pestañas. `mosaico` **no tiene** el control y fuerza summit
(`eqVrpDisplay:366` llama `mirovaEqVrp(r, innerKm, false)`).

Menor: no se persisten `onlyPrimaryPixel`, `showHotspots` ni `days-select` en `index`, ni la
escala en `diario` mientras sí las otras dos.
...
magnitud) y el footprint en `:3472`; la leyenda aplica `dim` al construirse (`:3012`).
`diario` sincroniza en dos `DOMContentLoaded` (`:701`, `:721`) y sus toggles recargan la
página, lo que los hace **inmunes por construcción** al bug de propagación. `mosaico`
corrige la etiqueta en `_syncMagBtn()` antes de renderizar. Ningún caso de «el botón dice
una cosa y el filtro hace otra» al cargar.

## 6. Controles sin afordancia — y una etiqueta que promete de más

- **`sensor-legend`** (`:572`): tiene `cursor:pointer` (`:54`) pero **ningún `title`** y, a
  diferencia de los otros cuatro grupos, **ningún `<label>` que lo nombre** — los demás
  dicen «Escala », «Distancia », «Pixels », «Magnitud »; éste flota a la derecha con
  `margin-left:auto`. Es el control más potente de la barra y el único que no se anuncia.
- **`hotspot-legend`** (`:589`): tres puntos VIIRS375 / VIIRS750 / MODIS casi idénticos a
  los de `sensor-legend`, y **no es clickeable**. Dos leyendas gemelas, una filtra y la
  otra no.
- **Etiqueta que promete de más**: el botón `◎ Núcleo F5'` y su tooltip (`:566`) no
  mencionan el alcance, pero `mirovaEqVrpCore:1082` devuelve el valor base si el sensor no
  es VIIRS I-band. En un volcán dominado por MODIS/VIIRS750 el toggle **no cambia nada** y
  no hay forma de saber por qué. `diario.html:100` sí lo dice: «(solo VIIRS375)».
- **Tooltip falso al usuario**: el `title` del grupo Distancia (`index.html:557`) afirma que
  ...
  contexto S129 marcaba el comentario `:809-811`; esto es la misma afirmación falsa pero
  **visible al operador**.
