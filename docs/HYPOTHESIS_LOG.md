...
- **Resolución**: run 34929024703, 40 pasadas procesadas, 0 descartes. **Veredicto pre-registrado: INDETERMINADO** en total, focal y nevado (el control falló). El verificador con contexto limpio (`experiments/_s141_fase1_probe/VERIFICADOR.md`) reprodujo los números pero **refutó las lecturas post hoc por estrato**: el patrón nevado lo sostiene sólo Nevados de Chillán, donde el píxel caliente de MIROVA está a 13 a 17 km del cráter (el pareo no filtraba la posición del foco de MIROVA); el orden de etapas pone en fila rutas que corren en paralelo; la máscara del Test 1 no es un test por píxel; y el control de conteo de hoy comparaba el núcleo F5 con el cúmulo. Ocho defectos del instrumento en total. **No justifica ningún brazo de A/B**; siguiente: instrumento v2 con muestra nueva. Detalle: `experiments/_s141_fase1_probe/RESULTADOS.md`.

## H_S141_VECINO_FOCO_V2: los vecinos tibios del foco que MIROVA suma los deja fuera un test identificable de la detección VIIRS 375, y se sabe por cuánto
...
- **Resolución** (run 34967596160, 38 pasadas; verificador con contexto limpio `experiments/_s141_fase1_probe_v2/VERIFICADOR_POST_CORRIDA.md`, reproducción re-corrida por el orquestador en S142):
  - Veredicto reproducido bit a bit (`_verificador/reproducir.py`: objeto y texto idénticos; C1 a C4 = 1,0 en 38 pasadas ok).
  - Lectura legítima: con el criterio pre-registrado, ningún test limitante es común a los volcanes focales y el fondo del cúmulo no cierra la brecha en ninguno. **No justifica ningún brazo de A/B** y no dice nada de los nevados.
  - Post hoc, declarado y sin reemplazar el veredicto (`_verificador/sensibilidad.py`): el HETEROGENEO mezcla dos rutas, cada una uniforme por dentro. En la contextual falla el dETI en 52 de 52 vecinos perdidos; en la del Test 1, la compuerta de BT en 18 de 18. El umbral efectivo es el piso C1 = 0,003 en los 70. El agregado da 0,643 contra el corte de 0,60 y sin Isluga baja a 0,592; sin Planchón-Peteroa pasaría a PATRON. Medido sólo sobre los vecinos perdidos, 4 de 5 volcanes quedan SIN_CONTRASTE. En 37 de 65 casos el índice del vecino elegido por BT es negativo.
  - Estrato nevado: una muestra en otra ventana del OSF no es viable. Los records de los 11 Tier A empiezan el 2025-02-15 (`experiments/_s142_nevado_muestra/PROPUESTA.md`).
  - Siguiente, sólo con pre-registro nuevo y verificador antes de correr: un v3 que separe las rutas y cuente por pasada, mida el contraste sobre los vecinos perdidos, elija vecinos por NTI y use una muestra nueva.

## H_S143_D22_D25: quitar la compuerta de temperatura (D22) y el fondo del anillo (D25) en VIIRS 375 recupera las noches que perdió S135 y acerca la magnitud a MIROVA sin fabricar publicaciones

- **Formulada**: S143 (2026-09-17), antes de correr. Pre-registro `docs/PREREGISTRO_AB_D22_D25_S143.md` v2 (verificador `PREREGISTRO_AB_D22_D25_S143_VERIFICADOR.md`, 18 hallazgos corregidos), brazos cerrados con el probe de las 12 noches (`experiments/_s143_probe_12_noches/`, con verificador antes y después de correr). Evaluador `experiments/_s143_evaluador/` con parámetros congelados; decisiones 1 y 1b de Nicolás del 2026-09-18 (cota de mismo objeto también en el brazo; posición `final_hotspot` en records `test1_roi`).
- **Hipótesis**: las 12 noches que perdió el brazo D de S135 son de la compuerta y del fondo; el brazo literal (D22 + D25 + segundo pase condicionado, sin `keep_peak`) las recupera, baja la publicación en negativos limpios y acerca la magnitud a 1.
- **Corrida**: VIIRS 375, 9 volcanes, 2026-06-01 a 2026-08-31, seis brazos. Runs 35266704955 y 35340495262 sobre el tag `pre-s143-ab-d22-d25`; tres jobs cortados por NASA (`SEARCH_CMR_TIMEOUT`, A64) repetidos en 35340956881 y 35449804356 sobre `pre-s143-ab-d22-d25-r2`, de código idéntico. Cobertura pareja: 9 volcanes, 0 desparejos en los dos tramos.
- **Estado**: resuelta como **no adoptar** (S143, 2026-09-19). Ningún brazo cumple los tres criterios.
- **Resolución** (`experiments/_s143_evaluador/RESULTADO_AB_S143.md`, `resultado_ab_s143.json`; verificador con contexto limpio `experiments/_s143_evaluador/VERIFICADOR_VEREDICTO.md`, veredicto SE SOSTIENE CON MATICES, recuento independiente en `verificador_veredicto/`):
  - Línea base del control en negativos limpios 0,917 sobre 1.170 pasadas, dentro del rango pre-registrado.
  - `literal`: cumple criterio 2 (0,917 a 0,547) y criterio 3 (magnitud 0,77 a 0,94); falla criterio 1 con 5 pérdidas, 0 sin la cota en el brazo. Recupera las 10 noches de S135 que el propio control confirma.
  - **Matiz del verificador sobre las 5 pérdidas**: no son otro objeto. Es la misma pasada con el mismo píxel más caliente; lo que "coincide" con MIROVA en el control es un píxel suelto a 2,2 a 2,8 km que conserva `keep_peak` (patrón A100) y cae por azar en el radio de MIROVA. La cota compara radios sin dirección y no decide cuál es el objeto. Con `final_hotspot` en todos los records el literal no perdería ninguna; no cambia el veredicto porque la posición se congeló antes de correr.
  - **Atribución** (literal menos cada ablación, recuento del verificador): todo el descenso en negativos viene de apagar `keep_peak` (literal menos `lit_keep_peak` −0,373); D22 y D25 empujan en contra (+0,202 y +0,052, intervalos lejos de cero). D22 recupera recall sólo con `keep_peak` apagado (`lit_con_compuerta` pierde 25 noches y 0 de las 12 de S135), a costa de ~20 puntos de sobre-publicación. D25 mueve la magnitud (`lit_sin_fondo` queda en 0,75). D2 casi nulo (6 de 3.400 records).
