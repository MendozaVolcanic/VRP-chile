"""
Audit F-S81-B' — gate intra-radio sobre second_pass_recapture (S85).

Compara 3 profiles sobre los 11 volcanes Tier A:
  - enabled : mirova_equivalent_f_s81_b_prime_2nd_pass_gate_enabled (gate ON)
  - disabled: mirova_equivalent_f_s81_b_prime_2nd_pass_gate_disabled (gate OFF)
  - baseline: mirova_equivalent (operacional post-S84 con F-S81-A ya activo)

Cambio vs audit S83 F-S81-A:
- Sensores reportados: MODIS + VIIRS (el gate aplica a los 3 process_*.py).
- Métrica nueva: `n_2nd_pass_recapture_agg` per-vol (suma de
  `diag_n_second_pass_recapture` en ventana). Esperamos reducción >50% en
  cirrus/glaciar (NdC, PP, Copahue, Chaiten).
- Criterio adopción: R3 violators MODIS ≤ 30 enabled (vs ~106 disabled).

Métricas por (profile × volcano × sensor):
  - N records con detección (pc.vrp_mw > 0)
  ...
  - ratio mediano pc.vrp_mw / mirova_VRP_MW (solo TPs)
  - R3: records con final_hotspot_source='eruption' y pc.centroid_dist_km > inner_radius_km
  - Σ diag_n_second_pass_recapture en ventana

Output: audit_results.md + audit_results.json en este mismo directorio.

Ventana: 2026-04-12 → 2026-05-26 (45 días, == workflow A/B run).

Uso:
    python experiments/_s85_f_s81_b_prime/audit.py
"""
...
OUT_DIR = ROOT / "experiments" / "_s85_f_s81_b_prime"
...
# Profiles a auditar
PROFILES = {
    "enabled": "mirova_equivalent_f_s81_b_prime_2nd_pass_gate_enabled",
    "disabled": "mirova_equivalent_f_s81_b_prime_2nd_pass_gate_disabled",
...
def load_profile_records(profile_dir: str, vol: str,
                         sensor_filter: str | None = None) -> list[dict[str, Any]]:
    """Carga records de un profile/volcán.

    sensor_filter: "MODIS", "VIIRS", o None para todos los sensores.
    """
    ...
    for r in data.get("records", []):
        ...
        out.append({
            ...
            "n_2nd_pass_recapture": int(r.get("diag_n_second_pass_recapture", 0) or 0),
        })
    return out
...
def audit_volcano(vol: str, profile_key: str, profile_dir: str,
                  mirova: dict, inner_radius: dict,
                  sensor: str = "MODIS") -> dict[str, Any]:
    """Audit un (volcano, profile, sensor). Retorna métricas."""
    recs = load_profile_records(profile_dir, vol, sensor_filter=sensor)
    csv_name = vol_csv_name(vol)
    alertas = mirova.get((csv_name, sensor), [])
    ...
    n_2nd_agg = sum(r["n_2nd_pass_recapture"] for r in recs)
    ...
    return {
        ...
        "n_2nd_pass_recapture_agg": n_2nd_agg,
        "inner_radius_km": inner_r,
    }


def main() -> int:
    print(f"[audit S85 F-S81-B'] window {WINDOW_START.date()} -> {WINDOW_END.date()} ({WINDOW_DAYS}d)")
    ...
    for profile_key, profile_dir in PROFILES.items():
        for vol in TIER_A:
            ...
            for sensor in ("MODIS", "VIIRS"):
                row = audit_volcano(vol, profile_key, profile_dir,
                                    mirova, inner_radius, sensor=sensor)
                all_rows.append(row)
    ...
    # Markdown
    md = ["# Audit F-S81-B' — gate intra-radio second_pass_recapture (S85)\n"]
    md.append(f"**Ventana**: {WINDOW_START.date()} → {WINDOW_END.date()} ({WINDOW_DAYS}d)\n")
    md.append(f"**Sensores**: MODIS + VIIRS (helper integrado en los 3 process_*.py)\n")
    ...
    # Tabla por (profile × sensor)
    for profile_key in PROFILES:
        for sensor in ("MODIS", "VIIRS"):
            md.append(f"\n## Profile: `{profile_key}` — sensor {sensor}\n")
            md.append("| Volcán | N rec | Detec | ALERTA | TP | FP | FN | Prec | Rec | F1 | FPs/mes | Ratio | R3viol | n_2nd_agg | inner |\n")
            md.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
            rows_p = [r for r in all_rows
                      if r["profile"] == profile_key and r["sensor"] == sensor]
            for r in rows_p:
                md.append(
                    f"| {r['volcano']} | {r['n_records']} | {r['n_detections']} | "
                    f"{r['n_alertas_mirova']} | {r['tp']} | {r['fp']} | {r['fn']} | "
                    f"{r['precision']} | {r['recall']} | {r['f1']} | "
                    f"{r['fps_per_vol_month']} | {r['ratio_median']} | "
                    f"{r['r3_violators']} | {r['n_2nd_pass_recapture_agg']} | "
                    f"{r['inner_radius_km']} |\n"
                )
            ...
            n_2nd_total = sum(r["n_2nd_pass_recapture_agg"] for r in rows_p)
            md.append(
    ...
    md.append("- R3 violators MODIS+VIIRS (`enabled`): **≤ 30 total** (vs ~106 disabled).\n")
    md.append("- Reducción `n_2nd_pass_recapture_agg` enabled vs disabled: **≥ 50%** "
              "per-vol en NdC/PP/Copahue/Chaiten.\n")
    md.append("- Cero pérdida TPs MIROVA (especialmente Lascar MODIS 25/25).\n")
    md.append("- Recall y precision sin regresión >5pp per-vol enabled vs disabled.\n")

    # Comparativa per-vol × sensor enabled vs disabled
    md.append("\n### Δ enabled - disabled (per-volcano × sensor)\n")
    md.append("| Volcán | Sensor | Δ TP | Δ FP | Δ FN | Δ R3 | Δ n_2nd_agg | Δ Recall | Δ Precision |\n")
    md.append("|---|---|---:|---:|---:|---:|---:|---:|---:|\n")
    by_key = {(r["volcano"], r["profile"], r["sensor"]): r for r in all_rows}
    for vol in TIER_A:
        for sensor in ("MODIS", "VIIRS"):
            e = by_key.get((vol, "enabled", sensor))
            d = by_key.get((vol, "disabled", sensor))
            if not e or not d:
                continue
            d_tp = e["tp"] - d["tp"]
            d_fp = e["fp"] - d["fp"]
            d_fn = e["fn"] - d["fn"]
            d_r3 = e["r3_violators"] - d["r3_violators"]
            d_2nd = e["n_2nd_pass_recapture_agg"] - d["n_2nd_pass_recapture_agg"]
            ...
            md.append(
                f"| {vol} | {sensor} | {d_tp:+d} | {d_fp:+d} | {d_fn:+d} | "
                f"{d_r3:+d} | {d_2nd:+d} | {rec_str} | {pre_str} |\n"
            )
