"""54_test1_ab/delta_report.py — A/B Test 1 integrated-ROI delta report (S25).

Mide la contribución del nuevo Path Test 1 (Coppola 2015 §2.2 Eq.1)
comparando _test1_enabled (treatment) vs _test1_disabled (control = mirror
operacional). Target Villarrica recall (6 ALERTAs MIROVA sub-pixel <600K).

Test 1 debería:
- Villarrica: capturar TPs nuevos (recall sube de ~0/6 hacia ≥4/6).
- 3 controles (Lascar/Lastarria/Tupungatito): summit y far casi estables
  (Test 1 NO debería disparar en pasadas sin actividad sub-pixel real).
...
PROFILES = ["_test1_enabled", "_test1_disabled"]
...
def main():
    ...
    # Veredicto Test 1
    lines.append("## Veredicto Test 1 integrated-ROI")
    lines.append("")
    summit_en = agg[PROFILES[0]]["summit_total"]
    summit_dis = agg[PROFILES[1]]["summit_total"]
    far_en = agg[PROFILES[0]]["far_total"]
    far_dis = agg[PROFILES[1]]["far_total"]

    # Per-volcano: Villarrica target, controles
    villarrica_en = rows_by_vol.get("Villarrica", {}).get(PROFILES[0])
    villarrica_dis = rows_by_vol.get("Villarrica", {}).get(PROFILES[1])
    if villarrica_en and villarrica_dis:
        delta_summit_match_v = villarrica_en["summit_match"] - villarrica_dis["summit_match"]
        n_refs_v = villarrica_en["n_refs"] or 0
        lines.append(f"- **Villarrica target**: TP_summit (matches MIROVA) {villarrica_dis['summit_match']} → {villarrica_en['summit_match']} ({delta_summit_match_v:+d}) sobre {n_refs_v} refs.")
        if n_refs_v > 0:
            recall_en = villarrica_en['summit_match'] / n_refs_v
            recall_dis = villarrica_dis['summit_match'] / n_refs_v
            lines.append(f"  - Recall summit: {recall_dis:.2f} → {recall_en:.2f}")

    # Controles: estabilidad
    lines.append("")
    lines.append("**Controles (no debería disparar en pasadas sin sub-pixel real):**")
    for vol in ["Lascar", "Lastarria", "Tupungatito"]:
        en = rows_by_vol.get(vol, {}).get(PROFILES[0])
        dis = rows_by_vol.get(vol, {}).get(PROFILES[1])
        if en and dis:
            d_summit = en["summit_total"] - dis["summit_total"]
            d_far = en["far_total"] - dis["far_total"]
            d_far_no = en["far_no_match"] - dis["far_no_match"]
            lines.append(f"- {vol}: Δ summit {d_summit:+d}, Δ far {d_far:+d}, Δ FP_far {d_far_no:+d}")

    lines.append("")
    lines.append("**Interpretación**:")
    lines.append("- Si Villarrica recall ≥ 0.50 con Test 1 ON, controles Δ FP_far ≤ +5% → **INTEGRAR a mirova_equivalent**.")
    lines.append("- Si Villarrica recall sube y controles inflan FPs > 10% → trade-off, evaluar refinamiento (k_sigma más alto, restricción summit).")
    lines.append("- Si Villarrica recall NO sube → bug en integración o algoritmo no captura en pipeline real (revisar vs POC offline).")
