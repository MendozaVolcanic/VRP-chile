...
def pasadas(arm):
    ...
    for v in V:
        for r in json.load(open(f'{B}/s143ab-{arm}-{v}/{v}.json',encoding='utf-8'))['records']:
            ...
            if r.get('final_hotspot_lat') is not None: pos=(r['final_hotspot_lat'],r['final_hotspot_lon'])
...
json.dump({a:[{**p,'t':str(p['t']),'mir':[{**m,'t':str(m['t'])} for m in p['mir']]} for p in R[a]] for a in ARMS},open('verif/fhall_pasadas_'+'_'.join(x[9:] for x in ARMS)+'.json','w'),default=str)
for a in ARMS[1:]:
