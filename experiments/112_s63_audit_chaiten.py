"""S63 Audit — Chaiten A/B kernel-bg.

Pre-condicion: workflow 26094823809 completado, JSONs en main:
- data/_local_kernel_bg_enabled/Chaiten.json (NEW)
- data/mirova_equivalent/Chaiten.json (LEGACY)

Uso:
  git pull --rebase origin main
  python experiments/112_s63_audit_chaiten.py

Verdict criterio:
- Recall NEW ≥ LEGACY (sin regresión)
- Ratio mediano NEW < LEGACY 10.28×
- Si NEW <3×: ADOPTAR local_kernel_bg=true Chaiten
"""
import io
import sys
...
def verdict(legacy, new):
    ...
    if recall_ok and ratio_ok and new['median_ratio'] < 3.0:
        print('  ==> ADOPTAR local_kernel_bg=true Chaiten')
    elif recall_ok and ratio_ok:
        print('  ==> MEJORA pero ratio > 3.0x. Considerar adopcion + investigacion residual.')
    else:
        print('  ==> NO ADOPTAR (verifica si patron Tupungatito-like)')


def main():
    legacy = audit('data/mirova_equivalent/Chaiten.json', 'Chaiten',
                   'Chaiten OPERACIONAL (LEGACY, sin fix)')
    new = audit('data/_local_kernel_bg_enabled/Chaiten.json', 'Chaiten',
                'Chaiten NEW (con kernel-bg fix)')
    verdict(legacy, new)
